...
from Models.CollideNet.hiera_utils import pretrained_model, conv_nd, do_pool, do_masked_conv, Unroll, Reroll
from Models.CollideNet.hfhub import has_config, PyTorchModelHubMixin
...
def hiera_base_plus_224_3_stages(**kwdargs):
    return Hiera(embed_dim=112, num_heads=2, q_pool=2, stages=(2, 3, 16), **kwdargs)

def hiera_base_plus_224_2_stages(**kwdargs):
    return Hiera(embed_dim=112, num_heads=2, q_pool=1, stages=(2, 3), **kwdargs)

def hiera_base_plus_224_1_stage(**kwdargs):
    return Hiera(embed_dim=112, num_heads=2, q_pool=0, stages=([2]), **kwdargs)


if __name__ == "__main__":
    model = hiera_base_16x224().cuda()
    model.head = nn.Identity()
    state_dict = torch.hub.load_state_dict_from_url("https://dl.fbaipublicfiles.com/hiera/mae_hiera_base_16x224.pth", map_location='cpu')
    model.load_state_dict(state_dict['model_state'], strict=False)
    model.head = Head(768, 1, dropout_rate=0.0)
    model = model.cuda()
    # model.head = nn.Identity()
    print(model(torch.zeros(2, 3, 16, 224, 224).cuda()).shape)